...
var orden=0;
	...
	function listarProductos(productos) {
	  var divagregar = document.getElementById("agregar");
	  divagregar.style.display = 'block';
	  var diveliminar = document.getElementById("eliminar");
...
function obtenerProductos() {
	  fetch('https://retoolapi.dev/WDvAh9/productos')
            .then(res=>res.json())
            .then(data=>{productos=data;productos.forEach(function(producto){
...
function incluirProducto(){
	...
	var miproducto = { image:imagen, price: precio, title:titulo, 
		category:categoria, description:descripcion
	};

	var addproducto;
	fetch("https://retoolapi.dev/WDvAh9/productos", {method:"POST", body:JSON.stringify(miproducto), headers:{'Content-Type':'application/json; charset=UTF-8',}})
	.then(response => response.json()).then(data => addproducto=data);
...
//Funcion para eliminar productos de la API
function deleteProducto(){
	var idproducto = document.getElementById("deleteid").value;
	if(idproducto <=0){
		alert("Id ingresado no es valido");
	}else{
		var deleteresult;
		fetch("https://retoolapi.dev/WDvAh9/productos/"+idproducto, {method:"DELETE"})
		.then(response => response.json()).then(data => deleteresult=data);
		deleteresult;
		alert("Se elimino el producto con ID #"+idproducto);
		location.reload();
	}
}